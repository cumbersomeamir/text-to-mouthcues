...
# Modify the mouth cue generation logic to extract only the "mouthCues"
def process_prompts_and_audio(num_prompts):
    ...
    for index, prompt in enumerate(prompts, 1):
        ...
        mouth_cues_response = send_for_mouth_cues(ogg_file_url)
        if mouth_cues_response:
            # Parse the "mouthCues" field from the response data
            mouth_cues = json.loads(mouth_cues_response['data'])['mouthCues']
            mouth_cues_list.append({
                "prompt": prompt,
                "mouth_cues": mouth_cues  # Only append the parsed mouthCues list
            })
